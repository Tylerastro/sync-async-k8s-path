...
# demo4 之後要開大 pool —— 一定要 export，不能只當指令前綴：
# ✗ DB_POOL_SIZE=100 docker compose up -d app   ← 變數只對「這一行」生效
# ✓ export DB_POOL_SIZE=100                     ← 對之後每一行 compose 指令都生效
# 原因：每個 docker compose 指令都會重新讀 compose.yaml 的 ${DB_POOL_SIZE:-5}，
#       沒 export 的變數下一個指令看不到 → 退回預設 5 → compose 發現 app 的
#       設定「不一致」就默默把它重建回 pool=5（真實踩過：四輪掃描全是假數據）
